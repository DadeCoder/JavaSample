...
public class GreetingController {
     ...
     * 这个类里面注入了SimpMessagingTemplete对象，后面动态发送消息时需要这个对象。
     第一个方法，表示服务器端可以接收客户端通过主题"/app/hello"发送过来的消息，
     客户端需要在主题"/topic/hello"上监听并接收服务器发回的消息。
     第二个方法道理相同，只是注意这里用的是@SendToUser注解，这就是发送给单一客户端的标志。
     本例中，客户端接收一对一消息的主题应该是"/user/"+userId+"/message"，这里的用户id可以是一个普通字符串，
     只要每个客户端都使用自己的id并且服务器端知道每个用户的id就行了。
